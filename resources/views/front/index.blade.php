        ...
        <div class="container max-w-[1130px] mx-auto flex flex-col items-center justify-center gap-[34px] z-10">
            ...
            <div class="flex w-full justify-center mb-[34px] z-10">
                <form action="{{ route('front.search') }}" method="GET"
                    class="group/search-bar p-[14px_18px] bg-lapakjmb-darker-grey ring-1 ring-[#414141] hover:ring-[#888888] max-w-[560px] w-full rounded-full transition-all duration-300">
    ...
    <div class="product-card flex flex-col rounded-[18px] bg-[#181818] overflow-hidden">
        ...
        <div class="p-[10px_14px_12px] h-full flex flex-col justify-between gap-[14px]">
            <div class="flex flex-col gap-1">
                ...
                <p class="bg-[#2A2A2A] font-semibold text-xs text-lapakjmb-grey rounded-[4px] p-[4px_6px] w-fit">
                    {{ $product->Category->name }}</p>
            </div>
            <div class="flex items-center gap-[6px]">
                <div class="w-6 h-6 flex shrink-0 items-center justify-center rounded-full overflow-hidden">
                    <img src="{{ \App\Helpers\ImageHelper::getUserAvatarUrl($product->Creator) }}" class="w-full h-full object-cover" alt="logo">
                </div>
                <a href="" class="font-semibold text-xs text-lapakjmb-grey">{{ $product->Creator ? $product->Creator->name : 'Unknown' }}</a>
            </div>
        </div>
    </div>
@empty
